...
extension Reducer {
    ...
    public func onChange<ChildState: Equatable>(
        of toLocalState: @escaping (State) -> ChildState,
        perform additionalEffects: @escaping (ChildState, ChildState, inout State, Action) -> Effect<Action>
    ) -> some Reducer<State, Action> {
...
struct ChangeReducer<Base: Reducer, ChildState: Equatable>: Reducer {
    ...
    init(
        ...
        perform: @escaping (ChildState, ChildState, inout Base.State, Base.Action) -> Effect<Base.Action>
    ) {
        self.base = base
